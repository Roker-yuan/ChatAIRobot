...
//            }
            ...
            if (null == topics || topics.isEmpty()) {
                logger.info("{} 本次检索未查询到待回答的问题", groupName);
                return;
            }
            ...
            // 3. 问题回复
            boolean status = zsxqApi.answer(groupId, cookie, String.valueOf(topic.getTopic_id()), answer, silenced);
            logger.info("{} 编号：{} 问题：{} 回答：{} 状态：{}", groupName, topic.getTopic_id(), topic.getQuestion().getText(), answer.trim(), status);
        } catch (Exception e) {
